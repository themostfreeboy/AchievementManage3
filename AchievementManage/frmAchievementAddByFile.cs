...
namespace AchievementManage
{
    ...
    {
        private int dgvCellRowIndexSuccess = -2;
        private int dgvCellColumnIndexSuccess = -2;
        private int dgvCellRowIndexFail = -2;
        private int dgvCellColumnIndexFail = -2;
        private int dgvSelect = -1;//标志选中的DataGridView为哪个控件(dgvDataSuccess=0还是dgvDataFail=1)
        ...
        private void dgvDataSuccess_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)//右键单击DataGridView(dgvDataSuccess)单元格的处理
        {
            if (e.Button == MouseButtons.Right)
            {
                dgvCellRowIndexSuccess = e.RowIndex;
                dgvCellColumnIndexSuccess = e.ColumnIndex;
                dgvSelect = 0;
                if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
                {
                    dgvDataSuccess.ClearSelection();//清除所有选中状态
                    dgvDataSuccess.Rows[e.RowIndex].Cells[e.ColumnIndex].Selected = true;//设置为选中状态
                    dgvDataSuccess.CurrentCell = dgvDataSuccess.Rows[e.RowIndex].Cells[e.ColumnIndex];//设置活动单元格
                    tsmniDeleteRow.Enabled = true;//导出时删除此行可点击
                    tsmniDeleteColumn.Enabled = true;//导出时删除此列可点击
                    cmsDelete.Show(MousePosition.X, MousePosition.Y);//弹出操作菜单
                }
                else if (e.RowIndex < 0 && e.ColumnIndex >= 0)
                {
                    dgvDataSuccess.ClearSelection();//清除所有选中状态
                    dgvDataSuccess.Columns[e.ColumnIndex].Selected = true;//设置为选中状态
                    tsmniDeleteRow.Enabled = false;//导出时删除此行不可点击
                    tsmniDeleteColumn.Enabled = true;//导出时删除此列可点击
                    cmsDelete.Show(MousePosition.X, MousePosition.Y);//弹出操作菜单
                }
                else if (e.RowIndex >= 0 && e.ColumnIndex < 0)
                {
                    dgvDataSuccess.ClearSelection();//清除所有选中状态
                    dgvDataSuccess.Rows[e.RowIndex].Selected = true;//设置为选中状态
                    tsmniDeleteRow.Enabled = true;//导出时删除此行可点击
                    tsmniDeleteColumn.Enabled = false;//导出时删除此列不可点击
                    cmsDelete.Show(MousePosition.X, MousePosition.Y);//弹出操作菜单
                }
                else if (e.RowIndex < 0 && e.ColumnIndex < 0)
                {
        ...
        private void dgvDataFail_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)//右键单击DataGridView(dgvDataFail)单元格的处理
        {
            if (e.Button == MouseButtons.Right)
            {
                dgvCellRowIndexFail = e.RowIndex;
                dgvCellColumnIndexFail = e.ColumnIndex;
                dgvSelect = 1;
                if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
                {
                    dgvDataFail.ClearSelection();//清除所有选中状态
                    dgvDataFail.Rows[e.RowIndex].Cells[e.ColumnIndex].Selected = true;//设置为选中状态
                    dgvDataFail.CurrentCell = dgvDataFail.Rows[e.RowIndex].Cells[e.ColumnIndex];//设置活动单元格
                    tsmniDeleteRow.Enabled = true;//导出时删除此行可点击
                    tsmniDeleteColumn.Enabled = true;//导出时删除此列可点击
                    cmsDelete.Show(MousePosition.X, MousePosition.Y);//弹出操作菜单
                }
                ...
                {
                    dgvDataFail.ClearSelection();//清除所有选中状态
                    dgvDataFail.Columns[e.ColumnIndex].Selected = true;//设置为选中状态
                    tsmniDeleteRow.Enabled = false;//导出时删除此行不可点击
                    tsmniDeleteColumn.Enabled = true;//导出时删除此列可点击
                    cmsDelete.Show(MousePosition.X, MousePosition.Y);//弹出操作菜单
                }
                ...
                {
                    dgvDataFail.ClearSelection();//清除所有选中状态
                    dgvDataFail.Rows[e.RowIndex].Selected = true;//设置为选中状态
                    tsmniDeleteRow.Enabled = true;//导出时删除此行可点击
                    tsmniDeleteColumn.Enabled = false;//导出时删除此列不可点击
                    cmsDelete.Show(MousePosition.X, MousePosition.Y);//弹出操作菜单
                }
        ...
        private void tsmniDeleteRow_Click(object sender, EventArgs e)//导出时删除此行
        {
            if (dgvSelect == 0)//dgvDataSuccess
            {
                dgvDataSuccess.Rows.RemoveAt(dgvCellRowIndexSuccess);
            }
            else if (dgvSelect == 1)//dgvDataFail
            {
                dgvDataFail.Rows.RemoveAt(dgvCellRowIndexFail);
            }
        }

        private void tsmniDeleteColumn_Click(object sender, EventArgs e)//导出时删除此列
        {
            if (dgvSelect == 0)//dgvDataSuccess
            {
                dgvDataSuccess.Columns.RemoveAt(dgvCellColumnIndexSuccess);
            }
            else if (dgvSelect == 1)//dgvDataFail
            {
                dgvDataFail.Columns.RemoveAt(dgvCellColumnIndexFail);
            }
        }
    }
}

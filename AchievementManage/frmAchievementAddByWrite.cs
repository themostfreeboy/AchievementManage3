...
namespace AchievementManage
{
    public partial class frmAchievementAddByWrite : Form
    {
        ...
        private void dgvData_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)//右键单击DataGridView单元格的处理
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                {
                    dgvData.ClearSelection();//清除所有选中状态
                    dgvData.Rows[e.RowIndex].Cells[e.ColumnIndex].Selected = true;//设置为选中状态
                    dgvData.CurrentCell = dgvData.Rows[e.RowIndex].Cells[e.ColumnIndex];//设置活动单元格
                ...
                {
                    dgvData.ClearSelection();//清除所有选中状态
                    dgvData.Columns[e.ColumnIndex].Selected = true;//设置为选中状态
                ...
                {
                    dgvData.ClearSelection();//清除所有选中状态
                    dgvData.Rows[e.RowIndex].Selected = true;//设置为选中状态
                    tsmniDeleteRow.Enabled = true;//导出时删除此行可点击
        ...
        {
            dgvData.Rows.RemoveAt(dgvCellRowIndex);
        }

        private void tsmniDeleteColumn_Click(object sender, EventArgs e)//导出时删除此列
        {
            dgvData.Columns.RemoveAt(dgvCellColumnIndex);
        }
    }
}
